...
BOOL CBoardSettings::OnInitDialog() 
{
    ...
    // NOTE: For Modbus RTU backend, some display values are simplified
    // AD Method displays the input type for Modbus devices
    if(ctx->NumAD >0) m_ADMethod0 = "Modbus RTU (HX711 + ADS1115)";
    if(ctx->NumAD >1) m_ADMethod1 = "N/A";
    ...
    // For Modbus, display raw int16_t range
    if(ctx->NumAD >0) m_ADRange0 = "int16_t (-32768 to +32767)";
    if(ctx->NumAD >1) m_ADRange1 = "N/A";

    if(ctx->NumAD >0) m_ADMaxChannel0.Format("%3d",ctx->ad.Channels[0]);
    if(ctx->NumAD >1) m_ADMaxChannel1.Format("%3d",ctx->ad.Channels[1]);
    
    if(ctx->NumDA >0 && ctx->da.Resolution[0]==12) m_DAResolution0 ="12 bit";
    if(ctx->NumDA >0 && ctx->da.Resolution[0]==16) m_DAResolution0 ="16 bit";

    // For Modbus GP8403 output
    if(ctx->NumDA >0) m_DARange0 = "0-10000 mV (GP8403)";
